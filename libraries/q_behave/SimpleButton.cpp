...
*/
SimpleButton::SimpleButton(uint8_t buttonPin, uint8_t buttonMode, bool _debounceMode, int _debounceDuration){
	pin=buttonPin;
  pinMode(pin,INPUT);
...
|| #
*/
void SimpleButton::pullup(uint8_t buttonMode)
{
	mode=BUTTON_PULLUP;
...
|| #
*/
void SimpleButton::pulldown(void)
{
...
*/
void SimpleButton::process(void)
{
...
|| @return true if button is pressed
*/
bool SimpleButton::isPressed(bool proc)
{
  if(proc) process();
...
|| #
*/
bool SimpleButton::wasPressed(bool proc)
{
  if(proc) process();
  return bitRead(state,CURRENT);
}
...
|| #
*/
bool SimpleButton::stateChanged(bool proc)
{
  if(proc) process();
...
|| #
*/
bool SimpleButton::held(unsigned int time /*=0*/)
{
  process();
...
|| #
*/
bool SimpleButton::heldFor(unsigned int time)
{
	if (isPressed()) 
  {
...
|| #
*/
void SimpleButton::setHoldThreshold(unsigned int holdTime)
{ 
...
*/
void SimpleButton::pressHandler(buttonEventHandler handler)
{
  cb_onPress = handler;
}
...
*/
void SimpleButton::releaseHandler(buttonEventHandler handler)
{
...
*/
void SimpleButton::clickHandler(buttonEventHandler handler)
{
  cb_onClick = handler;
}
...
*/
void SimpleButton::holdHandler(buttonEventHandler handler, unsigned int holdTime /*=0*/)
{
...
*/
unsigned int SimpleButton::holdTime() const { if (pressedStartTime!=-1) { return millis()-pressedStartTime; } else return 0; }
...
*/
bool SimpleButton::operator==(SimpleButton &rhs)
{
  return (this==&rhs);
}
